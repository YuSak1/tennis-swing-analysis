# TENNIS-SWING-ANALYSIS/flask_app/classification.py
...
def normalization(df):
    ...
    df_x = df_x.apply(
        lambda iterator: (
            (df.max().max() - iterator) / (df.max().max() - df.min().min())
        ).round(5)
    )
    df_y = df_y.apply(
        lambda iterator: (
            1 - (df.max().max() - iterator) / (df.max().max() - df.min().min())
        ).round(5)
    )
...
def create_features(df):
    ...
    df_out["right_elbow_dist"] = (
        df["right_elbow_y"] - (df["right_shoulder_y"] + df["left_shoulder_y"]) / 2
    )
...
def classification(mode):
    ...
    data_swing = (
        pd.read_csv("data/CSV/pose_detection.csv").drop("index", axis=1).iloc[0:90]
    )
    ...
    model_feature = load_model(
        "model_weight/tennis_swing_model_v5_feature.h5", compile=False
    )
    ...
    df_feature_ext = pd.read_csv("data/CSV/feature_ext_all_v5.csv").drop(
        "Unnamed: 0", axis=1
    )
    l_dist = []
    ...
    pred_result_video = (
        "static/videos/result_videos/" + most_similar_sample[:-4] + ".mp4"
    )
    ...
    if not quick_mode:
        ...
        df_feature_sub_input = pd.DataFrame(
            np.stack(
                [
                    features_sub1_input,
                    features_sub2_input,
        ...
        df_feature_sub_all = pd.read_csv("data/CSV/feature_ext_sub_all.csv").drop(
            "Unnamed: 0", axis=1
        )
        df_feature_sub_sample = (
            df_feature_sub_all[(df_feature_sub_all["name"] == most_similar_sample)]
            .iloc[:, :-2]
            .astype("float32")
        )
        ...
        for i in range(df_feature_sub_input.shape[0]):
            dist = abs(
                np.linalg.norm(
                    np.array(df_feature_sub_input.iloc[i])
                    - np.array(df_feature_sub_sample.iloc[i])
                )
            )
            l_dist.append(dist)
        ...
        msg_feature_sub = [
            "Height of your non-dominant hand.",
    ...
    if not quick_mode:
        return [
            ...
            pred_msg_m,
            pred_result_video,
            msg_feature_sub[smallest_indices[0]],
            msg_feature_sub[smallest_indices[1]],
            msg_feature_sub[smallest_indices[2]],
        ]
    else:
